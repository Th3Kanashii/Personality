...
@router.message(F.text.in_(["Молодіжна політика 📚", "Підтримка психолога 🧘",
                            "Громадянська освіта 🏛", "Юридична підтримка ⚖️"]))
...
@router.message(F.text.in_(["Молодіжна політика ✅", "Підтримка психолога ✅",
                            "Юридична підтримка ✅", "Громадянська освіта ✅"]))
async def subscribed(message: types.Message,
    ...
    if message.text == "Громадянська освіта ✅":
        await message.answer(text="Спілкування з волонтером не доступно",
                             reply_markup=cancel_subscription())
    else:
        await message.answer(text="Слова твої - вогінь, пиши мені, шаленій! 🤗",
                             reply_markup=cancel_subscription())
    await repo.users.update_user_subscription(category=message.text,
                                              user_id=message.from_user.id)
